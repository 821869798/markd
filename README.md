...
1. 按 `c` 新建分类（输入名称后回车提交）
2. **选中目标分类**（左栏单击或 Tab 切到左栏用 `j/k`）
3. 按 `a` 把目录加入该分类：输入为空则添加**当前所在目录**，也可输入任意路径
4. 也可以反向操作：选中已存在的书签按 `m`，把它移动到某个分类

最常用的快捷操作：进入想收藏的目录后运行 `mkd`，选中目标分类，按 `a` 直接回车——当前目录就进组了。
